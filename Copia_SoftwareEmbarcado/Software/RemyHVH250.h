...
/*Função para preenchimento dos frames da CAN na raça, APENAS PARA TESTE*/
...
private:
	float ModuleA = 0; /*! Module A temperature parameter without two's complement processing */
	float ModuleAProcessed = 0; /*! Module A temperature parameter processed though two's complement concept */
	float ModuleB = 0; /*! Module B temperature parameter without two's complement processing */
	float ModuleBProcessed = 0; /*! Module B temperature parameter processed though two's complement concept */
	float ModuleC = 0; /*! Module C temperature parameter without two's complement processing */
	float ModuleCProcessed = 0; /*! Module C temperature parameter processed though two's complement concept */
	float GateDriverBoard = 0; /*! Gate Driver temperature parameter without two's complement processing */
	float GateDriverBoardProcessed = 0; /*! Gate Driver temperature parameter processed though two's complement concept */
	float Byte[8]; /*! Data bytes to be received from CAN protocol (little endian) */
...
private:
	float ControlBoardTemperature = 0; /*! Control Board temperature parameter without two's complement processing */
	float ControlBoardTemperatureProcessed = 0; /*! Control Board temperature parameter processed though two's complement concept */
	float RTD1_Temperature = 0; /*! RTD1 temperature parameter without two's complement processing */
	float RTD1_TemperatureProcessed = 0; /*! RTD1 temperature parameter processed though two's complement concept */
	float RTD2_Temperature = 0; /*! RTD2 temperature parameter without two's complement processing */
	float RTD2_TemperatureProcessed = 0; /*! RTD2 temperature parameter processed though two's complement concept */
	float RTD3_Temperature = 0; /*! RTD3 temperature parameter without two's complement processing */
	float RTD3_TemperatureProcessed = 0; /*! RTD3 temperature parameter processed though two's complement concept */
...
private:
	float RTD4_Temperature = 0; /*! RTD4 temperature parameter without two's complement processing */
	float RTD4_TemperatureProcessed = 0; /*! RTD4 temperature parameter processed through two's complement concept */
	float RTD5_Temperature = 0; /*! RTD5 temperature parameter without two's complement processing */
	float RTD5_TemperatureProcessed = 0; /*! RTD5 temperature parameter processed through two's complement concept */
	float MotorTemperature = 0; /*! Motor temperature parameter without two's complement processing */
	float MotorTemperatureProcessed = 0; /*! Motor temperature parameter processed through two's complement concept */
	float TorqueShudder = 0; /*! Torque shudder parameter without two's complement processing */
	float TorqueShudderProcessed = 0; /*! Torque shudder parameter processed through two's complement concept */
	int Byte[8]; /*! Data bytes to be received from CAN protocol (little endian) */
...
private:
	float PhaseACurrent = 0; /*! Phase A current parameter without two's complement processing */
	float PhaseACurrentProcessed = 0; /*! Phase A current parameter processed through two's complement concept */
	float PhaseBCurrent = 0; /*! Phase B current parameter without two's complement processing */
	float PhaseBCurrentProcessed = 0; /*! Phase B current temperature parameter processed through two's complement concept */
	float PhaseCCurrent = 0; /*! Phase C current parameter without two's complement processing */
	float PhaseCCurrentProcessed = 0; /*! Phase C current parameter processed through two's complement concept */
	float DC_BusCurrent = 0; /*! DC Bus current parameter without two's complement processing */
	float DC_BusCurrentProcessed = 0; /*! DC Bus current parameter processed through two's complement concept */
	int Byte[8]; /*! Data bytes to be received from CAN protocol (little endian) */
public:
	CurrentInformation();
...
public:
    ...
    /**
	 * @brief Torque limit processing to be send to the motor
	 * @param TorqueLimit Torque limit value to configure the motor
	 */
     ...
     */
	void UpdateFrame(struct can_frame* frame, float Speed);
    /**
     * @param Position Byte position to be returned
     * @return Byte returned from CAN_DATA (little endian)
     */
	...
	void SetSpeedMode(struct can_frame* frame);
